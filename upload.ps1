...
if ($needSummary.Count -gt 0) {
    ...
    if ($configReady) {
        ...
        # שונה לגמרי מוורט: לא סיפור זורם עם משל בלב, אלא הצגה הלכתית מסודרת
        # של סוגיא, כמו שיעור עיון אמיתי. משמש בלי קשר למועד/פרשה (בפועל
        # כרגע רק מועדים, לפי הכלל ב-CLAUDE.md ש"בפרשות זה תמיד וורטים").
        $vortPromptIyun = "זהו תמלול (לא מדויק במאה אחוז - ייתכנו שגיאות הקלדה/הגייה קלות, התעלם מהן) של שיעור עיון הלכתי בעברית, העוסק בסוגיא או בשאלה הלכתית מסוימת. כתוב את תוכן השיעור כהצגה הלכתית מסודרת של הסוגיא - לא כוורט או כסיפור זורם. זהו הבדל מהותי מסגנון וורט רגיל: כאן הסגנון צריך להיות מדויק, לימודי ומובנה כמו שיעור עיון אמיתי, לא סיפורי ולא רגשי, ובלי משלים, סצנות או תיאורים ציוריים - אלא אם הם עצמם חלק מהסברה ההלכתית שהובאה בשיעור בפועל, ואז יש להביא אותם בקצרה ובענייניות ולא כ'סיפור מסגרת'. מבנה נדרש, בסדר הזה: (1) פתח בהצגה ברורה וממוקדת של השאלה או הסוגיא ההלכתית שבה עוסק השיעור - מהו בדיוק הנושא הנדון, מנוסח בבירור ככזה כבר במשפט הראשון או השני, בלי הקדמות. (2) הבא את המקורות, השיטות והדעות כפי שהם מופיעים בפועל בתמלול (פסוקים, גמרא, ראשונים, אחרונים, שיטות שונות) - לפי סדר ההתפתחות ההגיונית שבה הובאו בשיעור עצמו, כדיון מתפתח ולא כרשימה יבשה. (3) אם יש מחלוקת, קושי או מתח בין מקורות/שיטות כפי שהובא בשיעור - הצג אותו בבירור, כולל עיקרי הטיעונים לכל צד. (4) סיים במסקנה ההלכתית או בהכרעה כפי שהובאה בפועל בשיעור, מנוסחת בבירור ובקצרה - בלי מוסר השכל כללי ובלי סיכום רגשי. כתוב בפסקאות מסודרות, כל פסקה מקדמת שלב אחד בהתפתחות הדיון ולא חוזרת על פסקה קודמת. שפה תורנית מדויקת, בגוף שלישי, בלי פנייה ישירה מדומה לקהל ובלי ברכות/פניות בסוף. חשוב מאוד: הישען אך ורק על מה שמופיע בתמלול בפועל - אם שם, מקור, ציטוט או שיטה לא ברורים מהתמלול או נראים כשגיאת תמלול, אל תנחש ואל תמציא, נסח באופן כללי יותר או השמט. אל תזכיר בטקסט הסופי שמדובר בתמלול, ואל תתרגם לשפה אחרת. אל תוסיף כותרות, אל תשתמש בסימוני Markdown כמו כוכביות."
